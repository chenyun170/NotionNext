...
import { isHomepageListPost, isPublicContentPost } from '../utils/postVisibility'
...
export const getGlobalData = fetchGlobalAllData

const unresolvedPageBlockLogKeys =
  globalThis.__NOTIONNEXT_UNRESOLVED_BLOCK_LOG_KEYS__ ||
  new Set()
globalThis.__NOTIONNEXT_UNRESOLVED_BLOCK_LOG_KEYS__ = unresolvedPageBlockLogKeys
const siteDataBuildCache =
  globalThis.__NOTIONNEXT_SITE_DATA_BUILD_CACHE__ ||
  new Map()
globalThis.__NOTIONNEXT_SITE_DATA_BUILD_CACHE__ = siteDataBuildCache
/**
 * 获取全站数据; 基于Notion实现
...
export async function getSiteDataByPageId({ pageId, from }) {
  const cacheKey = `site_data_converted_${pageId}`
  if (shouldUseSiteDataBuildCache() && siteDataBuildCache.has(cacheKey)) {
    return siteDataBuildCache.get(cacheKey)
  }
  ...
  // 获取的数据格式与站点不同
  const siteData = await convertNotionToSiteData(
    pageId,
    from,
    deepClone(originalPageRecordMap)
  )

  if (shouldUseSiteDataBuildCache()) {
    siteDataBuildCache.set(cacheKey, siteData)
  }

  return siteData
}

function shouldUseSiteDataBuildCache() {
  return (
    process.env.BUILD_MODE === 'true' ||
    process.env.NOTIONNEXT_SITE_DATA_MEMORY_CACHE === 'true'
  )
}
...
async function convertNotionToSiteData(SITE_DATABASE_PAGE_ID, from, pageRecordMap) {
  ...
  const unresolvedPageBlocks = []
  const shouldLogRawPageBlocks = process.env.NOTIONNEXT_DEBUG_BLOCKS === 'true'
  ...
  for (let i = 0; i < pageIds.length; i++) {
    ...
    if (!pageBlock) {
      unresolvedPageBlocks.push(id)
      if (shouldLogRawPageBlocks) {
  ...
  if (
    unresolvedPageBlocks.length > 0 &&
    !shouldLogRawPageBlocks &&
    process.env.BUILD_MODE !== 'true'
  ) {
    const logKey = unresolvedPageBlocks.slice().sort().join(',')
    if (!unresolvedPageBlockLogKeys.has(logKey)) {
      unresolvedPageBlockLogKeys.add(logKey)
      console.warn(
        `Skipped ${unresolvedPageBlocks.length} unresolved Notion page blocks. Set NOTIONNEXT_DEBUG_BLOCKS=true to inspect raw blocks.`
      )
    }
  }
...
function handleDataBeforeReturn(db) {
  ...
  const safeTagOptions = Array.isArray(db?.tagOptions) ? db.tagOptions : []
  ...
  db.allNavPages = compactNavPages(shortenIds(db?.allNavPages) || [])
  //   db.allPages = cleanBlocks(db?.allPages)

  db.allNavPages = cleanPages(db?.allNavPages || [], safeTagOptions)
  db.allPages = cleanPages(db.allPages || [], safeTagOptions)
  db.latestPosts = cleanPages(db.latestPosts || [], safeTagOptions)
  // 必须在使用完毕后才能进行清理
  db.tagOptions = cleanTagOptions(safeTagOptions) || []
...
function cleanBlock(item) {
  ...
  if (pageBlock) {
    for (const i in pageBlock) {
      ...
      if (Array.isArray(pageBlock[i]?.value?.content)) {
        pageBlock[i].value.content = pageBlock[i].value.content.filter(
          id => pageBlock[id]?.value
        )
      }
...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  const defaultTitle = NOTION_CONFIG?.TITLE || BLOG.TITLE || '外贸获客情报局'
  const defaultDescription =
    NOTION_CONFIG?.DESCRIPTION ||
    BLOG.DESCRIPTION ||
    '外贸获客情报局专注海关数据、进口商查询、供应商关系分析和 AI 外贸工具，把真实贸易记录整理成可执行的客户开发线索。'
...
function compactNavPages(items) {
