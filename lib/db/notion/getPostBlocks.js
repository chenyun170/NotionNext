...
const shouldLogNotionRequests = process.env.NOTIONNEXT_DEBUG_REQUESTS === 'true'

const logNotionRequest = (...args) => {
  if (shouldLogNotionRequests) {
    console.log(...args)
  }
}
...
export async function getPageWithRetry(id, from, retryAttempts = 3) {
  if (retryAttempts && retryAttempts > 0) {
    ...
    try {
      ...
      logNotionRequest('[API<<--响应]', `耗时:${end - start}ms - from:${from}`)
      return pageData
    } catch (e) {
...
export const fetchInBatches = async (ids, batchSize = 100) => {
  ...
  for (let i = 0; i < ids.length; i += batchSize) {
    ...
    logNotionRequest('[API-->>请求] Fetching missing blocks', ids.length)
    const start = new Date().getTime()
