...
export const pickPostPreview = post => {
  if (!post) {
    return null
  }
...
export const pickSearchResultPost = post => {
  const preview = pickPostPreview(post)
  if (!preview) return null

  return {
    ...preview,
    tagItems: Array.isArray(post.tagItems) ? post.tagItems.slice(0, 4) : [],
    results: Array.isArray(post.results) ? post.results.slice(0, 4) : []
  }
}
...
export async function processPostData(props, from) {
  ...
  if (allPosts && allPosts.length > 0) {
    const index = allPosts.findIndex(
      post => post.id === props.post?.id || post.slug === props.post?.slug
    )
    ...
    props.prev = pickPostPreview(prevPost)
    props.next = pickPostPreview(nextPost)
    props.recommendPosts = getRecommendPost(
      props.post,
      allPosts,
      siteConfig('POST_RECOMMEND_COUNT')
    ).map(pickPostPreview).filter(Boolean)
  } else {
    props.prev = null
    props.next = null
