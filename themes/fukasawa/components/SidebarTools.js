      ...
      <div className="h-6 mt-0.5 flex items-center">
        ...
        {calcMode === 'conv' && (
          <div className="flex gap-1 w-full items-center">
            <select value={convType} onChange={e => setConvType(e.target.value)} className="bg-white dark:bg-black/40 border border-stone-200 dark:border-white/10 rounded h-5 text-[8px] outline-none">
        ...
        {calcMode === 'hs' && (
          <div className="flex gap-1 w-full items-center">
            <input className="flex-1 bg-white dark:bg-black/40 border border-stone-200 dark:border-white/10 rounded px-1 h-5 outline-none text-[9px]" placeholder={hsToGoogle ? "Google Code..." : "编码/产品名..."} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()}/>
            {/* HS/Google 引擎切换开关 */}
            <button 
